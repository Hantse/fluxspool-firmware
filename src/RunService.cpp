...
static bool parseAndStoreTokens(PreferenceService& prefs, const String& respJson) {
  ...
  long expiresIn = 0;
  if (root["expiresIn"].is<long>()) expiresIn = root["expiresIn"].as<long>();
  else if (root["expiresIn"].is<int>()) expiresIn = (long)root["expiresIn"].as<int>();
  else if (root["expiresIn"].is<const char*>()) expiresIn = atol(root["expiresIn"].as<const char*>());
...
void RunService::mqttSubscribeAll() {
  ...
  Serial.print("Subscribe confirm "); Serial.print(tConfirm); Serial.print(" -> "); Serial.println(ok ? "OK" : "FAIL");
}
...
void RunService::publishRegister() {
  ...
  Serial.print("Publish register -> "); Serial.print(t); Serial.print(" payload="); Serial.println(payload);
...
void RunService::publishStatusIfDue() {
  uint32_t nowMs = millis();
  if (nowMs - _lastStatusMs < _cfg.statusEveryMs) return;
  _lastStatusMs = nowMs;
  ...
  Serial.print("[STATUS] publish -> "); Serial.println(t);
  bool ok = _mqtt.publish(t.c_str(), payload.c_str());
  Serial.print("PUB -> "); Serial.print(t); Serial.print(" ok="); Serial.println(ok ? "true" : "false");
}

void RunService::publishTelemetryIfDue() {
  uint32_t nowMs = millis();
  if (nowMs - _lastTelemetryMs < _cfg.telemetryEveryMs) return;
...
void RunService::onRegisterConfirm(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.print("MQTT IN ["); Serial.print(t); Serial.print("] "); Serial.println(body);
  ...
  bool isReg = false;
  if (doc["IsRegister"].is<bool>()) isReg = doc["IsRegister"].as<bool>();
  else if (doc["isRegister"].is<bool>()) isReg = doc["isRegister"].as<bool>();
  else if (doc["ok"].is<bool>()) isReg = doc["ok"].as<bool>();
  ...
  Serial.print("Unsubscribe confirm "); Serial.print(topicOf("register/confirm")); Serial.print(" -> "); Serial.println(uok ? "OK" : "FAIL");
  ...
  Serial.print("Subscribe command "); Serial.print(tCmd); Serial.print(" -> "); Serial.println(ok1 ? "OK" : "FAIL");
  Serial.print("Subscribe topology "); Serial.print(tTopo); Serial.print(" -> "); Serial.println(ok2 ? "OK" : "FAIL");
...
void RunService::onTopologyResult(char* topic, byte* payload, unsigned int length) {
  ...
  if (probes.isNull()) probes = doc["probes"].as<JsonArray>();
  if (probes.isNull()) return;
  ...
  for (JsonVariant v : probes) {
    ...
    uint8_t mac[6];
    if (!EspNowService::parseMac(macStr, mac)) continue;
...
void RunService::loadTopologyFromNvs() {
  ...
  for (JsonVariant v : probes) {
    ...
    memcpy(p.mac, mac, 6);
    p.deviceKey = dkey;
    if (lmkHex.length() == 32 && EspNowService::hexTo16(lmkHex, p.lmk)) p.hasLmk = true;
